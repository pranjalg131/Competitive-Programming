...
int helper(int n, int k, int first){
    ...
    if(first == 0){
        int ans = ((helper(n-1,k,1) % MOD) + (helper(n-1,k,0) % MOD)) % MOD;
        dp[n][k][first] = ans;
        return ans;
    }
    else{ 
        int ans = ((helper(n-1,k-1,1) % MOD) + (helper(n-1,k,0) % MOD)) % MOD;
		dp[n][k][first] = ans;
        return ans;
    }
}
...
int getStrings(int n, int k){
    return (((helper(n-1,k-1,1) % MOD) + (helper(n-1,k,0) % MOD)) % MOD + ((helper(n-1,k,1) % MOD) + (helper(n-1,k,0) % MOD)) % MOD) % MOD;
}
